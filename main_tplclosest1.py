...
def main_run(algori):
    ...
    for network in tqdm(networks):
        ...
        runs = 10
        tot_tasks = 170
        open("../dblp-" + year + "/" + network + "-" + str(tot_tasks) + "-0-" + algori + "-results1.txt", "w").close()
        heading = results.get_heading()
        open("../dblp-" + year + "/" + network + "-" + str(tot_tasks) + "-0-" + algori + "-results1.txt", "a").write(
            heading + "\n")
        open("../dblp-" + year + "/" + network + "-" + str(tot_tasks) + "-0-" + algori + "-teams1.txt", "w").close()
        with open("../dblp-" + year + "/" + network + "-" + str(tot_tasks) + "-0.txt", "r") as file:
            ...
            for line in tqdm(file, total=n_lines):
                ...
                open("../dblp-" + year + "/" + network + "-" + str(tot_tasks) + "-0-" + algori +
                     "-teams1.txt", "a").write(",".join(sorted(team.experts)) + "\n")
                if crun % runs == 0:
                    ...
                    open("../dblp-" + year + "/" + network + "-" + str(tot_tasks) + "-0-" + algori + "-results1.txt",
                         "a").write(
                        record + "\n")
                    results.clean_it()
